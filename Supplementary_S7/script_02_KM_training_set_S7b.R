## Code for generating Supplementary figure S4b -->> construct risk groups and to check the survival of each risk group in Training dataset ##
...
# Convert to binary based on the cut off value
...
# Keep only the samples that are present in training dataset
...
df_final[,5:8] <- lapply(df_final[,5:8], as.character)
df_final[,5:8] <- lapply(df_final[,5:8], as.numeric)

# Calculate the number of patients in each risk group
df_final$risk_group <- rowSums(df_final[,5:8], na.rm = TRUE)
surv_meta <- df_final[,c(1:4, 9)]  # subset 
...
test <- ggsurvplot(
  ...
  pval.method = TRUE,
  size = 1.5,
  palette = c("purple","blue", "magenta", "forestgreen", "goldenrod1"),
  title = "",
  xlab = "Time (Months)",
  ylab = "Relapse Free Survival",
  legend.labs = c("0ev. (n = 8)","1ev. (n = 79)", "2ev. (n = 100)", "3ev. (n = 50)", "4ev. (n = 10)"),
...
test
ggsave("KMplot_for_all_features_training.png", test$plot, width = 7, height = 6, dpi = 1200,bg="white")

## Code to make Supplementary S3 f,g,h,i ## 
...
files <- list.files(path = './prog_ext_vali_GSE240671/all_files/', full.names = TRUE)  # 95 files
...
saveRDS(object = result2, file = 'result_GSE240671_TNBC_95_SAMPLE.rds')

###############################------------------------------------------

## Now visualize the expression of promoters in high and low risk group patients
# Here--- 
...
# Step-02 -> expression of pr_41492 in those patients and then make the boxplot of it
result2 <- readRDS('result_GSE240671_TNBC_95_SAMPLE.rds')
...
p <- p + stat_compare_means(aes(group = risk_group),
                            size=6,label.y = 7.2) + stat_n_text(size = 7,
                                                                color = "black",
                                                                fontface = "bold") 
p
ggsave("tumor_HUWE1pr.png", p, width = 5.7, height = 5.3, dpi = 1000)

####----

# Boxplots for FTX promoters-------------------------------------------------->>
subsetdf <- df[,c(1,3,4)]
...
p <- p + stat_compare_means(aes(group = risk_group),
                            size=6,label.y = 7.2) + stat_n_text(size = 7,
                                                                ...
                                                                fontface = "bold") 
p
ggsave("tumor_FTXpr.png", p, width = 5.7, height = 5.3, dpi = 1000)

##################################################################################################################

## FOR GENE EXPRESSION:-->>

# Now Checking the expression of gene - in the high and low category 
# Load raw counts
gene_exp <- as.data.frame(fread('output.txt'))
colnames(gene_exp) <- gene_exp[1,]
gene_exp <- gene_exp[-1,]
colnames(gene_exp)<- gsub("/rna_seq_pipeline/star/", "",colnames(gene_exp))
...
# Load raw counts of rest 68 samples:
gene_exp_2 <- as.data.frame(fread('output_68.txt'))
colnames(gene_exp_2) <- gene_exp_2[1,]
gene_exp_2 <- gene_exp_2[-1,]
colnames(gene_exp_2)<- gsub("/rna_seq_pipeline/star/", "",colnames(gene_exp_2))
colnames(gene_exp_2)<- gsub("_Aligned.sortedByCoord.out.bam", "",colnames(gene_exp_2))
rownames(gene_exp_2) <- gene_exp_2$Geneid
gene_exp_2 <- gene_exp_2[,-1]
gene_exp_2[] <- lapply(gene_exp_2, function(x) as.numeric(as.character(x)))

# Now merge the two gene expression metrices, 
final_count <- merge(gene_exp, gene_exp_2, by = 0)
rownames(final_count) <- final_count[,1]
final_count <- final_count[,-1]

x <- DGEList(counts=final_count )
x <- calcNormFactors(x,method = "TMM")
v <- voom(x, plot=F)
vMat <- v$E
metadata <- metadata[colnames(vMat),]
...
vMat_2 <- vMat_2[c( 'ENSG00000086758.17','ENSG00000230590.13'), ]   # Subset genes of interest
...
p <- ggboxplot(data_long, x = "risk_group", y = "expression", 
  ...
  ) + scale_y_continuous(limits = c(6, 12),expand = c(0,0))

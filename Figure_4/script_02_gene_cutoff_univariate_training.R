## Code to perform univariate survival analysis by calculating the statistically optimal cutoff value for genes on the training dataset ##
...
# Taking genes with >=2 promoters (multi-promoter genes)
filtered_df <- filtered_df %>%
  group_by(geneId) %>%
  filter(n() >= 2) %>%
...
## Taking relative promoter activity
...
# Step -> Read the training dataset
...
# Step -> In feature count matrix take only those samples that are present in training dataset
keep = intersect(colnames(vMat_2), training_dataset$Run )
length(keep)
vMAT_up <- vMat_2[, keep]

# Step -> Similarly, take only samples present in training datasets for metafile too
keep = intersect(surv_metadata$Run, training_dataset$Run )
surv_metadata <- surv_metadata[keep,]

# Step -> chunk: subsetting 1/3rd of sample (training dataset) and then calculating cutoff value for each gene and then storing it in a list

# Create a list to store the cut-off values for each run
cutoff_values_list <- vector("list", length = 5000) # total runs = 5000

# Run the code 5000 times
for (run in 1:5000) {
  selected_cols <- sample(1:ncol(vMAT_up), size = floor(ncol(vMAT_up) / 3)) #selecting random 1/3rd samples
  VMAT <- vMAT_up[, selected_cols]
  keep = intersect(colnames(VMAT), surv_metadata$Run )
  SURV_METADATA <- surv_metadata[keep,]
  ...
  gene_matrix <- perform_analysis_for_matrix(SURV_METADATA, t(VMAT))
  # Initiate a cut-off list
  cutoff_values <- list()
  for (i in 9:dim(gene_matrix)[2]) {
    mod_maxstat <- maxstat.test(Surv(RFS_time_Days, RFS_Status) ~ gene_matrix[, i],
                                data = gene_matrix, smethod = "LogRank", pmethod = c("none"))
...
# Save the cut-off list obtained in the form of RDS file
saveRDS(final_cutoff_values, file = 'gene_cut_off_sub_int_false.rds')
...
gene_matrix <- perform_analysis_for_matrix(surv_metadata, t(vMAT_up))

# Performing the univariate analysis on training dataset --- using the cut-off calculated above
mm_check_binary <- gene_matrix[, 9:dim(gene_matrix)[2]]
for (i in 1:ncol(mm_check_binary)) {
  mm_check_binary[, i] <- ifelse(gene_matrix[, i + 8] > final_cutoff_values[[i]], "high", "low")
}
mm_meta <- gene_matrix[, 1:8]
...
gene_fdr_p_values = p.adjust(gene_p_values1, method = 'fdr')
gene_res <- data.frame(Gene_names = colnames(mm_finaldf)[c(9:dim(mm_finaldf)[2])], Adj_pvalue = gene_fdr_p_values, PValue = gene_p_values1)
write.csv(gene_res, 'all_gene.csv')

# Find genes that are non-significant at P-value > 0.05
gene_nonsig_result <- filter(gene_res, PValue > 0.05)         

# Save the file
write.csv(gene_nonsig_result, 'gene_nonsig_pvalue_0.05.csv')

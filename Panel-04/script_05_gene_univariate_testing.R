## Code to use the cutoff value calculated from the training dataset on the validation (testing) dataset for performing univariate survival analysis ##
...
# Load the cut-off values
final_cutoff_values_1 <- readRDS('gene_cut_off_sub_int_false.rds')
...
for (i in 1:ncol(mm_check_binary)) {
  mm_check_binary[, i] <- ifelse(gene_matrix[, i + 8] > final_cutoff_values_1[[i]], "high", "low")
}
